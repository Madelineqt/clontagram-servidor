...
usuariosRouter.put('/:username', [jwtAuthenticate, transformarBodyALowercase], async (req,res) => {
  
  let usuarioacomprobar = req.body.username
  let usuarioautenticado = req.user.username
  let usuarioautenticadoid = String(req.user._id)
  let username = req.params.username
  console.log(usuarioacomprobar)
  ...
  if (usuario !== null){
  let usuarioid = String(usuario._id)
  
  if (usuarioautenticadoid !== usuarioid){
    log.warn(`Usuario ${usuarioautenticado} no es dueno del perfil con nombre ${username}`)
    res.status(401).send(`No tienes la misma id que ${username}, te jodes`)
    return
  } 
} 

  if (!usuario) {
    res.status(404).send(`El perfil ${username} no existe`)
  ...
  if (usuarioautenticado !== username){
    log.warn(`Usuario ${usuarioautenticado} no es dueno del perfil con nombre ${username}`)
    res.status(401).send(`No eres dueno de perfil ${username}, te jodes`)
    return
  }
  ...
  if (usuarioacomprobar !== usuarioautenticado){
    ...
    const alomejorestarepe = await usuarioController.obtenerUsuarioporUsuario(usuarioacomprobar)
    if (alomejorestarepe !== null){
      if (alomejorestarepe.username !== usuarioautenticado){
        log.info(`Usuario ${usuarioautenticado} ha fallado en la modificacion de su perfil porque el nuevo nombre estaba cogido`)
        res.status(401).send("Ese nombre ya está cogido")
        return
    }
  }
}
  usuarioController.editarUsuario(req.body._id, req.body.username, req.body.nombre, req.body.bio)
  .then(perfil =>{
    res.json(perfil)
    log.info(`perfil con nombre ${username} reemplazado`, perfil)
    return
  })
  .catch(err =>{
    log.error(`Excepcion en la modificacion del perfil ${username}`, err)
    res.status(500).send(`Excepcion en la modificacion del perfil ${username}`)
    return
  })
})
